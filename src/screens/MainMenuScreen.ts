...
export class MainMenuScreen implements Screen {
  ...
  mount(root: HTMLElement): void {
    ...
    root.querySelector<HTMLInputElement>('#player-name')?.addEventListener('input', (e) => {
      localStorage.setItem(NAME_KEY, (e.target as HTMLInputElement).value);
    });
  ...
  private renderModeGrid(): void {
    ...
    grid.querySelectorAll<HTMLElement>('.mode-card').forEach((card) => {
  ...
  private renderServerList(): void {
    ...
    list.querySelectorAll<HTMLElement>('.server-row').forEach((row) => {
