...
export function updateDebrisField(debris: Debris[], ships: Ship[], dt: number): Map<Ship, number> {
  const collectedBy = new Map<Ship, number>();

  for (const d of debris) {
    if (d.expired) continue;
    for (const ship of ships) {
      if (!ship.alive || ship.stats.magnetRange <= 0) continue;
  ...
  for (const ship of ships) {
    ...
    for (const d of debris) {
      ...
      if (Vector2.distance(ship.position, d.position) > PICKUP_RANGE) continue;
      const taken = Math.min(d.value, spaceLeft);
      if (taken <= 0) continue;
      ship.cargo += taken;
      d.value -= taken;
      collectedBy.set(ship, (collectedBy.get(ship) ?? 0) + taken);
      if (d.value <= 0) d.collected = true;
    }
  }
