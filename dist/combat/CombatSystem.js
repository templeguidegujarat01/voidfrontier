...
export class CombatSystem {
    ...
    tryFire(ship, nowMs) {
        if (!ship.firing || !ship.alive || !ship.canFire())
            return;
        ship.consumeFireCost();
        const p = new Projectile(ship.position, ship.angle, ship.stats.weaponProjectileSpeed, ship.stats.weaponDamage, ship.faction, ship.stats.weaponRange);
        ...
        for (const p of this.projectiles) {
            ...
            for (const ship of allShips) {
                ...
                if (ship.faction === p.owner)
                    continue; // no friendly fire within same faction
