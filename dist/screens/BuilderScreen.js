import { ModuleCategory } from '../ship/ModuleTypes.js';
...
export class BuilderScreen {
    ...
    `;
        ...
        root.querySelector('#save-loadout')?.addEventListener('click', () => {
            const nameInput = root.querySelector('#loadout-name');
    ...
    renderLoadoutList() {
        ...
        list.innerHTML = loadouts
            .map((l) => `<div class="loadout-row ${l.id === this.currentId ? 'loadout-row-selected' : ''}" data-id="${l.id}">${escapeHtml(l.name)}</div>`)
            .join('');
        list.querySelectorAll('.loadout-row').forEach((row) => {
    ...
    renderModuleGrid() {
        ...
        grid.querySelectorAll('.module-select').forEach((select) => {
            select.addEventListener('change', () => {
                const key = select.dataset.key;
                this.draft[key] = select.value;
                const desc = this.root?.querySelector(`#desc-${key}`);
                if (desc)
                    desc.textContent = getModule(select.value).description;
                this.renderStats();
            });
        });
    }
...
//# sourceMappingURL=BuilderScreen.js.map